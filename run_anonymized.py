...
from simulator.engine.person_anonymized import *
import argparse
parser = argparse.ArgumentParser()
parser.add_argument('--dataset', type=str, default='2019')  # 2019 for data only available in 2019, 2021 for data only available in 2021, 1921 for data available in both 2019 and 2021
parser.add_argument('--mode', type=int, default=0) # mode = 0 for learning based retrieval, 1 for evolving based retrieval
...
if __name__ == "__main__":
    ...
    folder = f"./database/{args.dataset}/"
    data = {"2019": available2019, "2021": available2021, "1921": available1921}
    for k in data[args.dataset]:
        with open(folder + str(k) + ".pkl", "rb") as f:
            att = pickle.load(f)
            P = Person(k, LLM())
            P.train_routine_list, P.test_routine_list, P.attribute, P.activity_cat, P.cat, P.domain_knowledge, P.neg_routines, P.activity_area, P.area_freq, P.full_area, P.first_split, P.loc_cat = att[0], att[1], att[2], att[3], att[4], att[5], att[6], att[7], att[8], att[9], att[10], att[11]
